...
void init_mouse(void) {
    ...
    status &= ~(1 << 5);      /* Clear disable auxiliary clock bit */
    ...
    /* Center mouse on the actual screen */
...
void mouse_handler(registers_t* r) {
    ...
    if (mouse_cycle == 3) {
        ...
        if (mouse_x >= (int)screen_width) mouse_x = (int)screen_width - 1;
        if (mouse_y >= (int)screen_height) mouse_y = (int)screen_height - 1;
