...
void D_PolysetDrawFinalVerts (finalvert_t *fv, int numverts)
{
	...
	// Also reject negative coordinates: a vertex barely off the top/left
	// edge would index zspantable/d_scantable with a negative subscript
	// and compute a wild pointer (kernel page fault).
...
// draw the point if splitting a leading edge
	...
	// Bounds guard: negative or off-screen midpoints would index the span
	// tables with a wild subscript and page-fault the kernel.
	if (new[0] < 0 || new[1] < 0 ||
		new[0] >= r_refdef.vrectright || new[1] >= r_refdef.vrectbottom)
		goto nodraw;
